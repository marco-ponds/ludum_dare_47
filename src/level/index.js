...
import { GRID_HEIGHT, GRID_WIDTH, getPositionFromRowAndCol, SPRITE_SIZE, SPRITE_SCALE, CURSOR_SCALE, getGridPositionFromCoordinates, INITIAL_TRACKS, areGridPositionsEqual, TRAIN_SCALE } from './grid';
import { DIRT, GRASS, getGrassSprite, CURSOR, TRACK, TRAIN_HEAD, TRAIN, TRACK_VERTICAL } from './sprites';
import CursorScript, { PLACE_TRACK_EVENT, TRACK_CLICK_EVENT } from './scripts/cursor';
...
export default class Intro extends Level {
    ...
    addCursor() {
        ...
        const position = getPositionFromRowAndCol(0, 0, SPRITE_SIZE, SPRITE_SCALE);
    ...
    addTrain() {
        ...
        const position = getPositionFromRowAndCol(0, 0, SPRITE_SIZE, SPRITE_SCALE, true);
    ...
    handleTrackClick = (event) => {
        ...
        const index = this.tracks.findIndex(track => areGridPositionsEqual(track.gridPosition, event.track.gridPosition));

        this.tracks[index].type = nextRotation;
        this.tracks[index].setTextureMap(TRACK_TYPES_TO_SPRITE_MAP[nextRotation]);
    ...
    buildInitialtracks() {
        for (let trackPosition of INITIAL_TRACKS) {
            const position = getPositionFromRowAndCol(trackPosition.row, trackPosition.col);
